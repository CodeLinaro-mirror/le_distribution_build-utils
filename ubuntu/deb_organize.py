...
def search_manifest_map_for_path(MANIFEST_MAP, SOURCE_DIR, path):
    ...
    to find a matching key by checking if the manifest key appears as a path component
    in the absolute path string. Keys are checked longest-first to ensure the most
    specific match wins (e.g. vendor/qcom/proprietary/foo beats vendor/qcom).

    Args:
    -----
    - MANIFEST_MAP (dict): The manifest map generated by `generate_manifest_map`.
    - SOURCE_DIR (str): The source directory (unused, kept for API compatibility).
    - path (str): The path to search for in the manifest map.

    Returns:
    --------
    - str: 'oss' or 'prop' from the manifest map if found, otherwise 'oss'.
    """
    if path:
        path_str = str(path).strip()
        # Sort keys longest-first so the most specific entry wins
        for key in sorted(MANIFEST_MAP.keys(), key=len, reverse=True):
            # Match key as a complete path segment sequence inside the absolute path
            if ('/' + key + '/') in path_str or path_str.endswith('/' + key):
